...
const PAGE_PATH = "/index_game_next15.html";

// スクリーンショット保存先ディレクトリ
const SCREENSHOT_DIR = "screenshots/test_next15";

test.describe("テトリス Next15", function () {
  ...
  test("初期ハイスコア: localStorageが空の場合はハイスコア0が表示される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => localStorage.removeItem("tetrisNext15HighScore"));
    await page.reload();
  ...
  test("初期ランキング: localStorageが空の場合はランキング欄に項目が表示されない", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => localStorage.removeItem("tetrisNext15Ranking"));
    await page.reload();
    await expect(page.locator("#ranking")).toBeVisible();
  ...
  test("再スタートで盤面・スコア・ピース数・最大コンボ・経過時間・ピース統計が初期化される(ハイスコア・ランキングは維持)", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => {
      localStorage.setItem("tetrisNext15HighScore", "500");
      localStorage.setItem("tetrisNext15Ranking", JSON.stringify([500, 300]));
    });
    await page.reload();
    await page.locator("#startBtn").click();
    await page.keyboard.press("Space");
  ...
  test("ハイスコアがlocalStorageに保存され、ページ再読み込み後も保持される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => localStorage.setItem("tetrisNext15HighScore", "1234"));
    await page.reload();
  ...
  test("ハイスコアをリセットボタンでリセットできる", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => localStorage.setItem("tetrisNext15HighScore", "999"));
    await page.reload();
  ...
  test("ランキング: 事前登録済みのスコアが降順で表示される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => {
      localStorage.setItem("tetrisNext15Ranking", JSON.stringify([100, 500, 300]));
    });
    await page.reload();
  ...
  test("ランキング: 上位5件のみ保持され、6件目以降は切り捨てられる", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => {
      localStorage.setItem("tetrisNext15Ranking", JSON.stringify([600, 500, 400, 300, 200, 100]));
    });
    await page.reload();
