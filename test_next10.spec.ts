...
const PAGE_PATH = "/index_game_next10.html";

// スクリーンショット保存先ディレクトリ
const SCREENSHOT_DIR = "screenshots/test_next10";

test.describe("テトリス Next10", function () {
  test("初期表示: ボード・HOLD欄・NEXTキュー3欄・スコア欄・ハイスコア欄・コンボ欄・ピース数欄・T-Spin数欄・全消し数欄・B2B欄・操作ボタン・ゴーストボタン・リセットボタンが表示される", async function ({ page }) {
  ...
  test("初期ハイスコア: localStorageが空の場合はハイスコア0が表示される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => localStorage.removeItem("tetrisNext10HighScore"));
    await page.reload();
  ...
  test("再スタートで盤面・スコア・コンボ・ピース数・T-Spin数・全消し数・B2B数が初期化される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.locator("#startBtn").click();
    await page.keyboard.press("Space");
    await page.waitForTimeout(100);
  ...
  test("ハイスコアがlocalStorageに保存され、ページ再読み込み後も保持される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => localStorage.setItem("tetrisNext10HighScore", "1234"));
    await page.reload();
  ...
  test("ハイスコアをリセットボタンでリセットできる", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => localStorage.setItem("tetrisNext10HighScore", "999"));
    await page.reload();
  ...
  test("B2B: 通常のハードドロップ(ライン消去なし)ではB2B数が加算されない", async function ({ page }) {
    ...
    // B2B未成立状態のハードコピー
    await page.screenshot({
      path: `${SCREENSHOT_DIR}/14_no_b2b_after_drop.png`,
      fullPage: true,
    });
  });

  test("B2B: 連続プレイ後もエラーなく盤面・B2B欄が表示される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.locator("#startBtn").click();
    ...
    await page.screenshot({
      path: `${SCREENSHOT_DIR}/15_after_extended_play.png`,
      fullPage: true,
    });
  });
  ...
  test("B2B: 再スタート時にB2B数がリセットされる", async function ({ page }) {
    ...
    await page.screenshot({
      path: `${SCREENSHOT_DIR}/16_b2b_reset_after_restart.png`,
      fullPage: true,
    });
  });
  ...
  test("全消し: 再スタート時に全消し数がリセットされる", async function ({ page }) {
    ...
    await page.screenshot({
      path: `${SCREENSHOT_DIR}/17_pcs_reset_after_restart.png`,
      fullPage: true,
    });
  });
});
