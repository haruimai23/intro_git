...
const PAGE_PATH = "/index_game_next7.html";

// スクリーンショット保存先ディレクトリ
const SCREENSHOT_DIR = "screenshots/test_next7";

test.describe("テトリス Next7", function () {
  test("初期表示: ボード・HOLD欄・NEXTキュー3欄・スコア欄・ハイスコア欄・コンボ欄・ピース数欄・操作ボタン・ゴーストボタン・リセットボタンが表示される", async function ({ page }) {
  ...
  test("初期スコア: スコア0・ライン0・レベル1・コンボ0・ピース数0が表示される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
  ...
  test("初期ハイスコア: localStorageが空の場合はハイスコア0が表示される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => localStorage.removeItem("tetrisNext7HighScore"));
    await page.reload();
  ...
  test("再スタートで盤面・スコア・コンボ・ピース数が初期化される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.locator("#startBtn").click();
    await page.keyboard.press("Space");
    await page.waitForTimeout(100);
  ...
  test("ハイスコアがlocalStorageに保存され、ページ再読み込み後も保持される", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => localStorage.setItem("tetrisNext7HighScore", "1234"));
    await page.reload();
  ...
  test("ハイスコアをリセットボタンでリセットできる", async function ({ page }) {
    await page.goto(BASE_URL + PAGE_PATH);
    await page.evaluate(() => localStorage.setItem("tetrisNext7HighScore", "999"));
    await page.reload();
